...
function buildIngredientPrompt(name: string): string {
  return `${name} as a raw cooking ingredient — depicted exactly as it would appear when pulled from a grocery bag or pantry shelf before cooking: whole, uncut, and uncooked (or in its typical store-bought form for processed items like canned goods, pasta, or sauces). Show the quantity that feels natural for how this ingredient is typically bought and used: if it is usually handled as individual items (like a bell pepper, an onion, a lemon, or a chicken breast), show a single one; if it is typically bought or used as a collection of many small pieces (like peas, french fries, blueberries, or pasta), show a natural small pile or handful. The subject should fill 80% of the frame. Isometric 3/4 view, 512x512 pixels, rendered in Retro-Pop art style. Thick clean black outlines. Vintage muted color palette: cream, teal, burnt orange, mustard yellow. Shadows rendered as halftone dot texture. Solid warm off-white background #F5F0E8. No text, no labels, no other objects, no plate or surface — just the ingredient itself.`
}
